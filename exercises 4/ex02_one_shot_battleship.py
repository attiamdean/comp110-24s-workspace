...
while row_guess > GRID_ROWS :
    row_guess: int = int(input((f"The grid is only {GRID_ROWS} by {GRID_COLUMNS}. Try again:")))

column_guess: int = int(input("Guess a column:"))
while column_guess > GRID_COLUMNS : 
    column_guess: int = int(input(f"The grid is only {GRID_ROWS} by {GRID_COLUMNS}. Try again:"))
