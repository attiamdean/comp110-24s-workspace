...
def add_by_kind(by_kind: dict[str, list[str]], new_plant_kind: str, new_plants: str) -> None:
    """Add plant under its kind."""
    if new_plant_kind in by_kind:
        by_kind[new_plant_kind] = new_plants
    else:
        by_kind[new_plant_kind] = []
        by_kind[new_plant_kind].append(new_plants)
...
def lookup_by_kind_and_date(plants_by_kind: dict[str, list[str]], plants_by_date: dict[str, list[str]], kind: str, month: str) -> str:
    """Return string with list of plants of a specific kind to plant in a specidic month."""
    assert kind in plants_by_kind
    assert month in plants_by_date
    # Get a list of all plants of the specific kind of imput
